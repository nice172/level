...
{block name="form"}
    ...
    <div class="form-group">
        ...
        <div class="col-sm-4 checkbox i-checks">
        	<label><input type="radio" value="0" {if condition="0 eq $user['level']"}checked="checked"{/if} name="level" /><i></i> 普通会员</label>
         	{foreach $level as $val}
                <label><input type="radio" value="{$val['level']}" {if condition="$val['level'] eq $user['level']"}checked="checked"{/if} name="level" /><i></i> {$val['name']}</label>
            {/foreach}
        </div>
    </div>
